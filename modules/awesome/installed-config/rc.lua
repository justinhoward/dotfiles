...
text_clock_widget = wibox.widget.textclock('%a %b %e %l:%M %P')
...
local function change_volume(by)
  ...
  volume = 'pactl set-sink-volume @DEFAULT_SINK@ ' .. by
  awful.spawn("sh -c '" .. unmute .. ' && ' .. volume .. "'")
end
...
globalkeys = gears.table.join(
  ...
  awful.key({ mod_key, 'Mod1' }, 'j', function () awful.screen.focus_relative(1) end,
            { description = 'focus the next screen', group = 'screen' }),
  awful.key({ mod_key, 'Mod1' }, 'k', function () awful.screen.focus_relative(-1) end,
  ...
  awful.key({ mod_key }, 'i', function () awful.spawn(browser) end,
            { description = 'open an internet browser', group = 'launcher' }),
  awful.key({ mod_key, 'Shift' }, 'i',
    function ()
    ...
    { description = 'open a private browsing window', group = 'launcher' }
  ),
  awful.key({ mod_key, 'Mod1' }, 'r', awesome.restart,
  ...
  awful.key({ mod_key, 'Mod1' }, 'h', function () awful.tag.incncol( 1, nil, true) end,
            { description = 'increase the number of columns', group = 'layout' }),
  awful.key({ mod_key, 'Mod1' }, 'l', function () awful.tag.incncol(-1, nil, true) end,
  ...
  -- Prompt
  awful.key({ mod_key }, 'r', function () awful.screen.focused().mypromptbox:run() end,
  ...
  awful.key({ mod_key }, 'Pause', function() awful.util.spawn('slock') end,
            { description = 'lock the screen', group = 'launcher' }),
  awful.key({ mod_key, 'Mod1' }, 'BackSpace',
    function ()
...
clientkeys = gears.table.join(
  ...
  awful.key({ mod_key, 'Mod1' }, 'space', awful.client.floating.toggle ,
            { description = 'toggle floating', group = 'client' }),
  awful.key({ mod_key, 'Mod1' }, 'Return', function (c) c:swap(awful.client.getmaster()) end,
            { description = 'move to master', group = 'client' }),
  awful.key({ mod_key }, 'o', function (c) c:move_to_screen(c.screen.index-1) end,
            { description = 'move to next screen', group = 'client' }),
  awful.key({ mod_key, 'Shift' }, 'o', function (c) c:move_to_screen() end,
...
for i = 1, 9 do
  globalkeys = gears.table.join(globalkeys,
    ...
    awful.key({ mod_key, 'Mod1', 'Shift' }, '#' .. i + 9,
      function ()
        if client.focus then
          local tag = client.focus.screen.tags[i]
...
clientbuttons = gears.table.join(
  ...
  awful.button({ mod_key }, 1, awful.mouse.client.move),
  awful.button({ mod_key }, 3, awful.mouse.client.resize)
)
