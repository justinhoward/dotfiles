#!/usr/bin/env sh

zstyle ':prezto:module:git:log:medium' format '%C(bold)Commit:%C(reset) %C(green)%h%C(red)%d%n%C(bold)Author:%C(reset) %C(cyan)%an <%ae>%n%C(bold)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'
zstyle ':prezto:module:git:log:oneline' format '%C(green)%h%C(reset) %s%C(red)%d%C(reset)'
zstyle ':prezto:module:git:log:cherry' format "%m %C(green)%h%C(reset) %s%C(red)%d%C(reset)"
